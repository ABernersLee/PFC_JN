function [mus,m] = pfc_theta_lock(thisdir,savefolder,toplot)
...
%           & vel(pfcspks(:,3))>velcutoff & armpos(pfcspks(:,3))==iarm & dirdat(pfcspks(:,3))==1,6))); 
      dat = pfcspks(pfcspks(:,2)==other_cells(icell) ...
          & vel(pfcspks(:,3))>velcutoff,6);
      mus(icell,1) = circ_r(deg2rad(dat)); 
       mus(icell,2:3) = circ_rtest(deg2rad(dat)); 
      mus(icell,4) = circ_mean(deg2rad(dat)); 
      mus(icell,5) = circ_kappa(deg2rad(dat)); 
      if toplot
         figure; hold on
         histogram([dat; dat+360],36)
         title([thisdir(1:end-4) ', Cell ' num2str(other_cells(icell)) ', pval = ' num2str(mus(icell,2))])
         if mus(icell,2)<.05
             toadd = '_sig';
         else; toadd = [];
         end
%          axis tight
        ylabel('PFC cell spikes')
        xlabel('HP theta phase')
         set(gca,'FontName','Arial')
         set(gcf,'renderer','Painters')
         helper_saveandclosefig([savefolder '/ThetaBasics/SingleCells_' thisdir(1:end-4) '_Cell ' num2str(other_cells(icell)) toadd])
