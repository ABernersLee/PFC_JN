function Figure3_Internal_combo(dirs,igroup,savefolder)
% uses the linear error to find times when the internal representation is
% on a different arm from the animal, uses that error to make the 2d
% internal plots
...
d2 = dir('*.mat');
binsize = 20;
One_D_Filter=fspecial('gaussian',[3 1],2);
Two_D_Filter=fspecial('gaussian',[2 2],1);
velcutoff = 5;
EstBin = .1; 
cellcutoff = 3;
spikecutoff = 5;
...
tocalc = false;
for id = 1:size(d2,1)
    clear FRinternal OpenFRinternal
    thisdir = d2(id).name;        
    ...
    if tocalc 
        ...
        if 1 % get the error indicies
            ...
            load(d2(id).name,'RP_pSSDarm','RP_SSDarm','hp_cells','hpinterneurons','InFR','OutFR','hp_cells','hpinterneurons','spikedata','pos','binpos','armpos','armposindex','vel','other_cells')
...
%                             end
                ...
                helper_saveandclosefig(['E:\XY_matdata\Figures\ForPaper\Figure3\related\1DInternalcombo_' vellab{ivel} '_' d2(id).name(1:end-4) '_Occ']) 
                        close gcf
...
%             end
    ...
    % Plot cells
    if 0 %toplot
        load(thisdir,'spikedata','hp_cells','other_cells','hpinterneurons','pos','FRinternal','RP_pSSDarm')    
...
%             dat = filter(One_D_Filter,1,dat);
    ...
    clearvars -except id err d2 binsize SigmaField One_D_Filter velcutoff EstBin cellcutoff spikecutoff vellab loclab newcol toplot tocalc Two_D_Filter
...
%                             end
            ...
            title('Difference in Occupancy')
            set(gcf,'Position',[2562           6         494         879])
                helper_saveandclosefig(['E:\XY_matdata\Figures\ForPaper\Figure3\related\2DInternalcombo_' vellab{ivel} '_' d2(id).name(1:end-4) '_Occ']) 
   ...
   if toplot
    ...
    %         r2 = corr(dat1(:),dat3(:),'rows','complete');        
            ...
            helper_saveandclosefig(['E:\XY_matdata\Figures\ForPaper\InternalFields\2DInternalDiffArm_' cellid '_' d2(id).name(1:end-4) '_Cell' num2str(icell)]) 
