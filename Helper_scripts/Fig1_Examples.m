...
thisdir = d2(id).name;
label = 'RP';
clearvars -except label thisdir d2 id
...
load(thisdir,'HP_Ripple','HP_RippleRaw')
load(thisdir,'armposindex')
for ilook = 2:size(tosave,1)
    figure; hold on; 
%     imagesc(post(postind(tosave(ilook)):postind(tosave(ilook)+1)-1,:)'); 
    ...
    title(num2str(ilook))
%     helper_saveandclosefig(['E:\XY_matdata\Figures\Example_id10_Fig1_ExampleReplay' num2str(ilook) '_new'])
    helper_saveandclosefig([savefolder '\Example_id10_Fig1_ExampleReplay' num2str(ilook) '_new'])
    
    rip = HP_RippleRaw(HP_RippleRaw(:,1)>=ts(tosave(ilook),1) & HP_RippleRaw(:,1)<=ts(tosave(ilook),2),:);
    
    figure; hold on
    plot(rip(:,2),'k','LineWidth',3)
    title(num2str(ilook))
    helper_saveandclosefig([savefolder '\Example_id10_Fig1_ExampleReplay' num2str(ilook) '_ripple1'])
    
    figure; hold on
    plot(rip(:,3),'k','LineWidth',3)
    title(num2str(ilook))
    helper_saveandclosefig([savefolder '\Example_id10_Fig1_ExampleReplay' num2str(ilook) '_ripple2'])
end

 %%
 ii = 31;
    rip = HP_RippleRaw(HP_RippleRaw(:,1)>=ts(lookat(ii),1) & HP_RippleRaw(:,1)<=ts(lookat(ii),2),:);
    
    figure; hold on
    plot(rip(:,2),'k','LineWidth',3)
    title(num2str(ilook))
    helper_saveandclosefig([savefolder '\Example_id10_Fig1_ExampleReplay_generic_ripple1'])
    
    figure; hold on
    plot(rip(:,3),'k','LineWidth',3)
    title(num2str(ilook))
    helper_saveandclosefig([savefolder '\Example_id10_Fig1_ExampleReplay_generic_ripple2'])
...
figure; hold on
a = subplot(3,1,1);
plot(pos(:,1),binpos(:,1),'.','MarkerSize',15,'Color','k') %,[.5 .5 .5])
...
for ilook = 1:size(tosave,1)    
    plot(pos(i(ilook),1),binpos(i(ilook),1),'d','MarkerSize',25,'Color',ic(ilook,:),'MarkerFaceColor',ic(ilook,:))
end
% axis tight
set(gca,'xlim',xl)
xt = get(gca,'xtick');
set(gca,'xticklabel',round(xt/60))
plot(xl,[find(armposindex(:,1),1,'last') find(armposindex(:,1),1,'last')],'k--')
plot(xl,[find(armposindex(:,2),1,'last') find(armposindex(:,2),1,'last')],'k--')
set(gca,'ylim',[1 max(binpos)])
set(gca,'ytick',max(binpos),'yticklabel',3.23)
ylabel('Position (m)')
% xlabel('Time (Min)')
set(gca,'FontSize',18)
...
helper_saveandclosefig([savefolder '\Example_id10_ExampleReplayPosSpikesDecoding'])
...
helper_saveandclosefig([savefolder '\Example_id10_ExampleReplayPosSpikesDecoding_legend'])