...
for iarm = 1:3
    ...
    for ioth = 1:2
        ind2 = Th(:,3)==iarm & Th(:,5)==oths(ioth);
        dat(:,iarm,ioth,1) = nansum(nansum(PFCthetaspikes_binned(:,modind,ind2),2),3)./(sum(modind)*binsize*sum(ind2));
        dat(:,iarm,ioth,2) = nanmean(nansum(PFCthetaspikes_binned(:,modind2,Th(:,3)==iarm & Th(:,5)==oths(ioth)),2)./(sum(modind2)*binsize),3);
    end
end
