function [SSDarm_real_laps,SSDarm_fake_laps] = plot_PFC_ArmReplay_triggered_expected_ReviewerComment(thisdir,withmodpatch,smoothsize,savefolder,id,toplot,igroup,sig)
...
load(thisdir,'binpos','armposindex','laps_coverspace','spikedata','vel','dirdat','armpos')
VelThresh = 5;
Spike=[spikedata(:,2) spikedata(:,1) binpos(spikedata(:,3)) vel(spikedata(:,3)) dirdat(spikedata(:,3)) armpos(spikedata(:,3)) laps_coverspace(spikedata(:,3))];
InFR = NaN(length(other_cells),length(armposindex),max(laps_coverspace)); OutFR = InFR;
Filter=fspecial('gaussian',[1 20],2);
for iarm = 1:3    
    Pos = binpos(armpos==iarm);        
    vel1 = vel(armpos==iarm);        
    dirdat1 = dirdat(armpos==iarm);
    laps_coverspace1 = laps_coverspace(armpos==iarm);
    for i=1:length(other_cells)
        for ilap = 1:max(laps_coverspace)                
             t=find(Spike(:,4)>VelThresh & Spike(:,1)==other_cells(i) & Spike(:,5)==0 & Spike(:,6)==iarm & Spike(:,7)==ilap);
            if length(t)~=1
                FR=60*histc(Spike(t,3),min(Pos):1:max(Pos))./histc(Pos(vel1>VelThresh & dirdat1==0 & laps_coverspace1==ilap),min(Pos):1:max(Pos));   
            else
                FR=60*histc(Spike(t,3),min(Pos):1:max(Pos))'./histc(Pos(vel1>VelThresh & dirdat1==0 & laps_coverspace1==ilap),min(Pos):1:max(Pos));   
            end        

            FR(isnan(FR) | isinf(FR))=0;
            InFR(i,armposindex(:,iarm),ilap)=filter2(Filter,FR');  

             t=find(Spike(:,4)>VelThresh & Spike(:,1)==other_cells(i) & Spike(:,5)==1 & Spike(:,6)==iarm & Spike(:,7)==ilap);
            if length(t)~=1
                FR=60*histc(Spike(t,3),min(Pos):1:max(Pos))./histc(Pos(vel1>VelThresh & dirdat1==0 & laps_coverspace1==ilap),min(Pos):1:max(Pos));   
            else
                FR=60*histc(Spike(t,3),min(Pos):1:max(Pos))'./histc(Pos(vel1>VelThresh & dirdat1==0 & laps_coverspace1==ilap),min(Pos):1:max(Pos));   
            ...
            FR(isnan(FR) | isinf(FR))=0;
            OutFR(i,armposindex(:,iarm),ilap)=filter2(Filter,FR');  
        end
    end
end
% 
fr = NaN(size(other_cells,1),3,size(OutFR,3));    
FR = InFR+OutFR;
for iarm = 1:3
...
if sig 
    torun = other_cells_touse(:,igroup) & RP_pSSDarm<.05 & ~isnan(RP_pSSDarm);
else
    torun = other_cells_touse(:,igroup);
end
SSDarm_real_laps = [];
SSDarm_fake_laps = [];
    
for ilap = 1:size(fr,3)
    SSDarm_real = [];
    SSDarm_fake = [];
    for icell = 1:length(pfc)
        ...
        dif = abs(mbS-repmat(fr(icell,:,ilap),[nump 1]));
        [~,mmm] = min(sum(dif,2));
        ...
        for iarm = 1:3        
            ...
            mm(iarm,:) = nanmean(m-b); %SSD
    %         mm(iarm,:) = nanmean((m-b)./(m+b)); %SSD2
        end
    ...
    %                 ms(iarm,:) = nanmean((m-b)./(m+b)); %SSD2
                ms(iarm,:) = nanmean(m-b); %nanmean(m-b)/nanmean(m+b); %nanmean(m-b); %SSD
            end
            for ic = 1:size(combos,1)
        ...
        SSDarm_real = [SSDarm_real; RP_SSDarm(icell)]; % RP_SSDarm2(icell)];
        SSDarm_fake = [SSDarm_fake; SSD_obs];
    ...
    %         plot(dl(dl(:,2)==pfc(icell),1),jj+toadd,'.','Color',newcol(iarm,:),'MarkerSize',5)
            ...
            if iarm == 1
                ...
                title(['Rat ' thisdir(1) ', Day ' num2str(dday(id)) ', Cell ' num2str(pfc(icell)) ' Lap ' num2str(ilap)])
            end
    ...
    %             end
        ...
        helper_saveandclosefig([savefolder '\ArmReplayTriggered\RP_' thisdir(1:end-4) '_smooth' num2str(smoothsize) '_' num2str(withmodpatch) 'modpatch_Cell' num2str(pfc(icell)) labaddto '_Expected_lap' num2str(ilap)])
        end
    end
    SSDarm_real_laps = cat(2,SSDarm_real_laps, SSDarm_real);
    SSDarm_fake_laps = cat(2,SSDarm_fake_laps, SSDarm_fake);
