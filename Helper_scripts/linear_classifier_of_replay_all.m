...
for id =  1:size(d2,1)
    if ignoresessions == 1 && ismember(id,[1,7,10]) %reviewer comment, 10/12/2020, ignoring sessions where rat hasn't explored all paths
        continue
    end
...
if ignoresessions
    savelab = [savelab '_IgnoreNoPathSessions'];
end
