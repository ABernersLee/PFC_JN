...
for id = 1:size(d2,1)
    thisdir = d2(id).name;
    load(thisdir,'pos','armpos','linposcat',[label '_replay_singlebothjoint'],[label  '_replay_replayarm'], ...
        [label '_PFCreplayspikes_binned'],[label '_PFCreplayspikes_list'],'other_cells',[label  '_replay_shuffle_p']...
        ,[label  '_replay_corr'],[label  '_replay_stnd'],[label  '_replay_maxjump'],[label  '_replay_dirbias'],[label  '_replay_armcov'])
...
%     fwd_rev(corr<0) = 1; %inbound
%     fwd_rev(corr>0) = 2; %outbound
        
    [~,ii] = histc(stnd(:,1),pos(:,1));
    maxarm = NaN(size(dirbias));
    for iarm = 1:3
        maxarm(replayarm==iarm) = .9*max(linposcat(armpos==iarm));
    end
    ratatend = linposcat(ii)>=maxarm;
    rataway = (replayarm==armpos(ii) & corr<0);    
     
%     fwd_rev(ratatend & rataway) = 1; %going away from rat (while he's at the end)
%     fwd_rev(ratatend & ~rataway) = 2; %all others (while rat at end)
    
    fwd_rev(ratatend & rataway) = 1; %going away from rat (while he's at the end)
    fwd_rev(ratatend & ~rataway & replayarm==armpos(ii)) = 2; %going towards the rat (while he's at the end)
