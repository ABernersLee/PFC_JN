function [accdiff1,accdiff2,accall,accSall] = decode_choice_by_thetaphase_hp(thisdir,numshuff,indinds,indinds2)
load(thisdir,'times_armon_thetaof_headingarm_lap_thetahalf_all','HPthetaspikes_binned');
...
%     dat = squeeze(nansum(HPthetaspikes_binned(:,modind,touse),2)); %./(sum(modind)*binsize));
    dat2 = squeeze(nansum(HPthetaspikes_binned(:,modind2,touse),2)); %./(sum(modind2)*binsize));
%     dat(sum(dat,2)<5,:) = [];
%     dat2(sum(dat2,2)<10 | sum(dat2,2)>600,:) = [];
    dat2(mean(dat2,2)<.2,:) = [];
...
helper_saveandclosefig(['E:\XY_matdata\Figures\ForPaper\Classify\Sessions\' thisdir(1) thisdir(3:end-4) '_HP_Local_' num2str(indinds2(1)) 'ms_to_' num2str(indinds2(2)) 'ms_numshuff_' num2str(numshuff)])
...
%         touse = Th(:,3)==iarm & Th(:,6)==ilap;
    ...
    dat2 = squeeze(nansum(HPthetaspikes_binned(:,modind,touse),2)); %./(sum(modind)*binsize));
%     dat2 = squeeze(nansum(HPthetaspikes_binned(:,modind2,touse),2)); %./(sum(modind2)*binsize));
%     dat(sum(dat,2)<5,:) = [];
    dat2(mean(dat2,2)<.2,:) = [];
%     dat2(sum(dat2,2)<5,:) = [];
...
helper_saveandclosefig(['E:\XY_matdata\Figures\ForPaper\Classify\Sessions\' thisdir(1) thisdir(3:end-4) '_HP_NonLocal_' num2str(indinds(1)) 'ms_to_' num2str(indinds(2)) 'ms_numshuff_' num2str(numshuff)])
