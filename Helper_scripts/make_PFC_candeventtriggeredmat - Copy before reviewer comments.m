...
for iE = 1:size(E,1)    
    k = (rand(1,nS)-window(2)); %[[0 to 1] - 2] should be -2 to -1 
    j = repmat((PFCs(PFCs(:,3)==iE,1)),[1 nS])+ones(sum(PFCs(:,3)==iE),1)*k; % randomly adding -1 to -2
    j(j<-window(1)) = 1+j(j<-window(1)); 
    j(j>(window(2))) = 1-j(j>(window(2)));
