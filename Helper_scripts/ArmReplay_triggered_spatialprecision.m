function ArmReplay_triggered_spatialprecision(dirs,igroup,savefolder)
cd(dirs.homedir)
d2 = dir('*.mat');
...
for id = 1:size(d2,1)
    [prec_hp,prec_pfc,sig_pfc,toplot,toplotind] = get_ArmReplay_triggered_spatialprecision(d2(id).name,igroup);
    hp = cat(1,hp,prec_hp);
...
helper_saveandclosefig([savefolder '\Figure2\precision_fields'])
...
%                 armind = find(armposindex(:,iarm));
    ...
    helper_saveandclosefig([savefolder '\Figure2\precision_fields_full_' smbinlab{smoothbinned}])
end
...
for isig = 1:2
    ...
    for iraw = 1:2
        ...
        axis tight
        set(gcf,'renderer','Painters')
        helper_saveandclosefig([savefolder '\Figure2\precision_selectivity_prob_ ' rawlab{iraw} '_' siglab{isig}])
    end
end
%%
for iraw = 1:2
    [r,p] = corr(sig(~isnan(sig)),pfc(~isnan(sig),iraw));
    figure; hold on;
    text(0,1,['Correlation between sig pval and spatial selectivity ' rawlab{iraw} ':'])
    text(0,.5,['r = ' num2str(r) ' p = ' num2str(p)])
    axis off
    helper_saveandclosefig([savefolder '\Figure2\precision_selectivity_corr_ ' rawlab{iraw}])
end
...
for iraw = 1:2
    ...
    [p,~,stats] = ranksum(hp(:,iraw),pfc2(:,iraw),'tail','right');
    h1 = [h1; sum(hp(:,iraw)>3)];
...
%     b(1).FaceColor = [1 1 1];
    ...
    plot(1:length(h1),h1./sum(h1),'.-','color',[.5 .5 .5],'LineWidth',2,'MarkerSize',20)
    ...
    xlabel('Spatial Selectivity (Max/Mean)')
    set(gca,'FontSize',18)
    title([rawlab{iraw} ' ' siglab{isig} ',n=' num2str(size(hp,1)) ',' num2str(size(pfc2,1)) ',p=' num2str(p) ',z=' num2str(stats.zval)])
    axis tight
    set(gcf,'renderer','Painters')
    helper_saveandclosefig([savefolder '\Figure2\precision_selectivity_prob_ ' rawlab{iraw} '_' siglab{isig} '_2_nobar'])
end