function [behavior,behave_change_log, behave_ind] = GUI_make_behavior_day(linposcat,armpos,linposcatnan,params)
...
while okay == 0
    ...
    for j = 1:size(Track_Places,1)+1   %behavior
        for i = 1:size(Track_Places,2) %arm
            ...
            plot(linposcatnan(behavior(:,i)==j),pos2(behavior(:,i)==j),'.','MarkerEdgeColor',c(j,:))
        end    
    end
    axis tight
...
behave_change_log = NaN(size(behavior,1),6);
behave_log = behave_change_log;
behave_ind = NaN(size(behavior,1),1);

for irun = 1:size(params.Run_Times,1)
    runind = linposcatnan(:,1)>=params.Run_Times(irun,1) & linposcatnan(:,1)<=params.Run_Times(irun,2);

    p = linposcatnan(runind,:);
    b = nansum(behavior(runind,:),2);
    ...
    behave_change_log(runind,:) = laps2ch;    
    behave_log(runind,:) = laps2;
    behave_ind(runind,:) = ilaps2;

end

plotind = 1:size(linposcat,1);
c = varycolor(size(behave_log,2));
figure; hold on
for j = 1:size(behave_log,2)   %behavior
    plot(plotind(behave_ind==j),linposcat(behave_ind==j),'.','MarkerEdgeColor',c(j,:))
end
