function make_singleandjointreplay_fromPosterior(thisdir,label)
load(thisdir,[label '_InMatrix'],[label '_OutMatrix'],[label '_Index'],[label '_CandEventTimes'],'armposindex')
...
WCcutoff = .3;
timecutoff = round(.05/.005); %to make it 50ms, changed to 40ms
MAPcutoff = 5; %new @ 4, was 5
% armcovcutoff = .5;
JDcutoff = .4;
gauss_x=-20:1:20;
...
replayqual.seqjointjump = []; replayqual.stnd = []; replayqual.seqmax = [];

CandEventAssign = NaN(length(Index),1);
for c = 1:length(Index)-1
    ...
    for iarm = 1:max(armpos)
        if sum(cut(:,iarm))>=timecutoff
            ...
            if ~isempty(iis)
                ...
                for i = 1:size(iis,1)
                    Mat = D(iis(i,1):iis(i,2),armpos==iarm)';
                    I2=sum([1:size(Mat,1)]'*ones(1,size(Mat,2)).*Mat);
                    armcovs(i) = range(I2)/sum(armpos==iarm);
                    mjump(i) = max(diff(I2))/sum(armpos==iarm);
                    segment = Mat';
    ...
    if ~isempty(ireplays)
        ireplays = ireplays(ireplays(:,4)>WCcutoff & ireplays(:,6)<JDcutoff,:); % & ireplays(:,5)>armcovcutoff,:);
    end
    joi = [];
        ...
        if length(unique(ireplays(:,3)))>1              
             ...
             for jr = 1:size(jj,1)    
                 if diff(jarm(jr,:))==0
                   ...
                   jind = [min(ireplays(jj,1)) max(ireplays(jj,2))];
    ...
    if ~isempty(joi)
        ...
        replayqual = addreplay(D,jind_all(joi,1),jind_all(joi,2),jord_all{joi},3,replayqual,jarm(joi,:),InMatrix,OutMatrix,times,jj);
        for ijarm = 1:2            
            thisarm = jarm(joi,ijarm);         
            % add both singles
            replayqual = addreplay(D,jjind(joi,1,ijarm),...
                jjind(joi,2,ijarm),find(armpos==thisarm),2,replayqual,jarm(joi,ijarm),InMatrix,OutMatrix,times,[]);            
        end
    else
        if ~isempty(ireplays)
            CandEventAssign(c) = 1;
            if size(ireplays,1)==1
                ...
                replayqual = addreplay(D,ireplays(1),ireplays(2),find(armpos==ireplays(3)),1,replayqual,ireplays(3),InMatrix,OutMatrix,times,[]);                
            elseif size(ireplays,1)>1
                %add best of the single replays
                bestarm = find(max(abs(ireplays(:,4)))==abs(ireplays(:,4)));
                replayqual = addreplay(D,ireplays(bestarm,1),ireplays(bestarm,2),find(armpos==ireplays(bestarm,3)),1,replayqual,ireplays(bestarm,3),InMatrix,OutMatrix,times,[]);                
            end
        elseif ~isempty(allpassedtime)            
            %add best of the bad replays
             if size(allpassedtime,1)==1
                ...
                CandEventAssign(c) = 1;
                replayqual = addreplay(D,allpassedtime(1),allpassedtime(2),find(armpos==allpassedtime(3)),1,replayqual,allpassedtime(3),InMatrix,OutMatrix,times,[]);           
            elseif size(allpassedtime,1)>1
                %add best of the bad replays if one is best               
                bestarm = find(max(abs(allpassedtime(:,4)))==abs(allpassedtime(:,4)));
                if length(bestarm)==1
                    CandEventAssign(c) = 1;
                    replayqual = addreplay(D,allpassedtime(bestarm,1),allpassedtime(bestarm,2),find(armpos==allpassedtime(bestarm,3)),1,replayqual,allpassedtime(bestarm,3),InMatrix,OutMatrix,times,[]);             
                else
                   CandEventAssign(c) = 0; 
                end
            end
...
save(thisdir,[label  '_replay_singlebothjoint'],[label '_CandEventAssign'],[label  '_replay_times'],...
    ...
    [label  '_replay_seqtimes'],[label  '_replay_seqreplayindex'],[label  '_replay_stnd'],'-append')
