...
currpos = NaN(size(achange,1),1);
for itheta = 1:size(achange,1)
    prospectivecycles(itheta,:) = nanmean(lappro(ind(itheta,1):ind(itheta,2),:)); % new 4/22/19 was mode
    currpos(itheta,:) = binpos(ind(itheta,1),1);
    outcycle(itheta,:) = nanmean(lapout(ind(itheta,1):ind(itheta,2),:)); % new 4/22/19 was mode
end
