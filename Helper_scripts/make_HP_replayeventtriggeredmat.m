function make_HP_replayeventtriggeredmat(thisdir,label)

load(thisdir,[label '_replay_stnd'],'hp_cells','hpinterneurons','spikedata',[label '_replay_singlebothjoint'],[label  '_replay_shuffle_p']...
    ,[label  '_replay_corr'],[label  '_replay_maxjump'])

other_cells = setdiff(hp_cells,hpinterneurons);
...
eval([label '_HPreplayspikes_list = PFCreplayspikes_list;'])
eval([label '_HPreplayspikes_binned = PFCreplayspikes_binned;'])
save(thisdir,[label '_HPreplayspikes_binned'],[label '_HPreplayspikes_list'],'-append')

disp('Done with make_HP_replayeventtriggeredmat')
