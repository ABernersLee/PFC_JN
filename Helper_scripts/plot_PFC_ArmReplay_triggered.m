function plot_PFC_ArmReplay_triggered(thisdir,label,withmodpatch,smoothsize,savefolder,id)
disp('Start plot_PFC_ArmReplay_triggered')
...
p_SSD2 = p_SSD; %(p_SSD<.05); 

%(Cand_sig_modu_include(:,3)==1); clear other_cells;
...
%             yl(1) = max([yl(1) 0]);
    ...
    helper_saveandclosefig([savefolder '\ArmReplayTriggered\' label '_' thisdir(1:end-4) '_smooth' num2str(smoothsize) '_' num2str(withmodpatch) 'modpatch_Cell' num2str(pfc(icell)) labaddto])
end
disp('Done with plot_PFC_ArmReplay_triggered')