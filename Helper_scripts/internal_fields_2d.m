function [SSDarm,alldat,alldatS] = internal_fields_2d(thisdir,EstBin,cellcutoff,cmcutoff,toplot,numshuff)
...
for ivel = 1:2
    ...
    for ilocal = 1:2
        ...
        for itt = 1:length(t2)
            s = Spike(Spike(:,1)>=t2(itt) & Spike(:,1)<(t2(itt)+.1),2);
            spks(:,itt) = histc(s,other_cells);
        end
        
        for idp = 1:size(armposindex,1)
            fr(:,idp,ivel,ilocal) = sum(spks(:,dp2==idp),2)./(sum(dp2==idp)*.1); %EstBin);
        end
