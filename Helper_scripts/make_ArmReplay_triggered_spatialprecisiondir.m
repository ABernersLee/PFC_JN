function make_ArmReplay_triggered_spatialprecisiondir(thisdir,label)
disp('Starting make_ArmReplay_triggered_spatialprecision')

load(thisdir,[label '_replay_singlebothjoint'],[label '_replay_corr'],[label '_replay_replayarm'], ...
...
%%
hp = hp_cells(~ismember(hp_cells,hpinterneurons)); 
stnd2save = stnd(~isnan(replayarm),:);
replayarm2save = replayarm(~isnan(replayarm));
corr2 = corr1(~isnan(replayarm));
precDIR = []; prec1DIR = []; armsDIR = [];
for idir = 1:2
    if idir == 1
        stnd2 = stnd2save(corr2<0,:);
        replayarm2 = replayarm2save(corr2<0,:);
    elseif idir == 2
        stnd2 = stnd2save(corr2>0,:);
        replayarm2 = replayarm2save(corr2>0,:);
    end
...
%         disp(['Done hp cell ' num2str(ihp) ' of ' num2str(length(hp))])
    ...
    spks(~mod(I,2),:)=[]; % only takes spikes within the candidate events
    ...
    precDIR = cat(3,precDIR,prec);
    prec1DIR = cat(3,prec1DIR,prec1);
    armsDIR = cat(3,armsDIR,arms);
    disp(['Done dir ' num2str(idir)])
end
%%
...
% toplotind(:,3:4) = arms;
spatialprecisiondir.prec = precDIR;
spatialprecisiondir.toplot = prec1DIR;
spatialprecisiondir.toplotind = armsDIR;
save(thisdir,'spatialprecisiondir','-append')
disp('Done with make_ArmReplay_triggered_spatialprecisiondir')
