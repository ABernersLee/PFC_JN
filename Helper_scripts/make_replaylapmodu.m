function [lapmodu,cellmodu] = make_replaylapmodu(thisdir,label,ilaptype,corrcut,jumpcut)
...
for icell = 1:length(pfc)
    for ilap = 1:max(laps)
        ...
        if sum(lapind & abs(Rcorr)>corrcut & maxJD<jumpcut)>0
            m = nanmean(sum(PFCreplay(icell,modind,lapind & abs(Rcorr)>corrcut & maxJD<jumpcut),2));
            b = nanmean(sum(PFCreplay(icell,baseind,lapind & abs(Rcorr)>corrcut & maxJD<jumpcut),2)./2);        
